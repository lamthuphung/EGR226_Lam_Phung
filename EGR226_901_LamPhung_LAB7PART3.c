...

{
    ...
    LCD_init();
    int i, j;
    char banner[20] = "LABORATORY OVER";

    while (1)
    {
        //Set cursor movement
        commandWrite(0b00000110);
        for (j = 0; j < strlen(banner); j++)
        {
            delay_ms(50);
            //this for loop will loop through any string entered and print it to the screen
            for (i = j; i < strlen(banner); i++)
            {
                dataWrite(banner[i]);
                delay_micro(10);
            }
        ...
        delay_ms(500);
        //change cursor position to row three column 1
...
void printString(char* string)
{
    ...
    //this for loop will cycle through any sting entered and print values to the screen.
