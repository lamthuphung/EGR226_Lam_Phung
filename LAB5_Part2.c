...
* Project: LAB 5
* File: LAB 5 - Part 2
* Description: Sequencing colored LEDs using a timer and pushbutton
**************************************************************************************/
...
void main(void)
{
    ...
    case 0:
        ...
        SysTick_delay(100); //wait 100ms
        P4->OUT &= ~BIT0;
        P4->OUT &= ~BIT1;
        P4->OUT &= ~BIT2;
        if (deBounce_Press() == 1)  //Check if the button is pressed
        {
            SysTick_delay(50);  //wait 50ms
            if (deBounce_Press() == 1)   //After making sure the button was pressed, the program move to state 1.
            state = 1;
        }
        break;
    ...
    case 1: //Case 1: Green LED ON, Yellow LED OFF, Red LED OFF.
        ...
        {
            SysTick_delay(50);      //wait 50ms
            if (deBounce_Press() == 1)   //After making sure the button was pressed, the program move to state 2.
            state = 2;
        }
        break;
    ...
    case 2: //Case 2: Green LED OFF, Yellow LED ON, Red LED OFF.
        ...
        {
            SysTick_delay(50);
            if (deBounce_Press() == 1)   //After making sure the button was pressed, the program move to state 3.
            state = 3;
        }
        break;
    ...
    case 3: //Case 3: Green LED OFF, Yellow LED OFF, Red LED ON.
        ...
        {
            SysTick_delay(50);  //wait 50ms
            if (deBounce_Press() == 1)    //After making sure the button was pressed, the program move back to state 1.
           state = 1;
        }
        break;
    }
